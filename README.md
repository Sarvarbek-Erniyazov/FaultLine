...
**Experimental programme closed (2026-09-20).** Eight pre-registered gates were run (ADR-0021 → ADR-0028). The decisive test (F7', ADR-0026) **passed its instrument control nine of nine and returned H1' SUPPORTED**: read with a text-aware linear head, the joint model beats the telemetry-only model by +0.017 to +0.023 AUPRC on every seed. ADR-0026 §1 withdrew two backbone ablations (`joint_status_raw`, `joint_no_txt`); the H1' outcome made them revisitable under a new registration against a named write-up sentence. ADR-0027 ran both through the H1' read-out: `joint_no_txt` is INCONCLUSIVE (its instrument gate passed) and `joint_status_raw` is NOT EVALUABLE (its gate failed), so neither write-up sentence is decided, and no further arm is registered. ADR-0028 evaluated calibration and abstention: the uncertainty signal is informative (Gate A passed on all three arms), but graceful degradation under channel loss was not established (H2 INCONCLUSIVE), and the probabilities under-read the test-period event rate by about half. Nothing in this repository is a deployed system.

Total GPU cost of the runs that wrote a timing sidecar beside their checkpoints: **12.6 GPU-hours** on a single NVIDIA RTX 4060 (8 GB), over 98 records in eleven run directories. That is a floor for the programme, not its total: the telemetry ladder, the text pretraining and the order-blind comparators wrote no sidecar and are timed only in their own reports under `reports/data/`. Two runs came afterwards: F8-2 (ADR-0027) adds **3.37 GPU-hours** (1.02 h pretraining, 1.24 h probes, 1.11 h scorings; `reports/data/ablation_gate_v0_20260923.md`), and F9-2 (ADR-0028) adds **2.24 GPU-hours** (9 clean validation and 12 masked test scorings; ADR-0028's outcome in `docs/DECISIONS.md`). Itemised, 12.6 h + 3.37 h + 2.24 h gives a floor of **18.2 GPU-hours** over the whole programme.
...
| Instrument audit | Fifteen recorded cases where an evaluation instrument or a stated premise, not the model, was the defect — each with its counterfactual. | `docs/INSTRUMENT_AUDIT.md` |
...
### 5. Calibration and abstention: implemented and evaluated; graceful degradation not established (ADR-0028, forward-in-time)

Dropping one to three core channels at inference costs the joint probe at most 0.0085 AUPRC — about half its lift over base rate on the seed where that worst case falls, and an eighth or less on the other two. Too mild to test H2, so ADR-0028 registered a severity ladder that masks k of the 12 core channels (k = 2, 4, 6, 8) on the three-seed ensemble of the text-aware read-out, abstaining on seed disagreement at an operating point fixed on the 2021 validation split. Gate A passed on all three arms: seed disagreement orders risk better than a random ordering, so abstention is evaluable. Gate B was barely met: masking eight channels damages the ranking, ΔAUPRC −0.0039 [−0.0082, −0.0001]. **H2 is INCONCLUSIVE**: at k = 8 coverage falls, Δcov −0.0797 [−0.0841, −0.0753], and selective risk still rises, Δrisk +0.0053 [+0.0033, +0.0074], an interval that straddles the registered smallest effect of 0.005. Abstention gives up coverage but cannot be shown to hold selective risk. Across the ladder, ECE falls as k rises because masking raises the mean probability toward the base rate, not because masking calibrates. The `tel_only` row is ADR-0025 control (iii) on the R0 windows, not F3's `tel_only` comparator on the 1,872-token windows.

Calibration is a separate finding. The prior-corrected probabilities under-read the test-period event rate by about 2×, before and after Platt recalibration on validation (mean probability 0.019–0.022 against a base rate of 0.0388). The cause is mainly the post-2021 rise in event rate, which no recalibration fitted on earlier data can anticipate. Without recalibration on recent data, these scores are rankings, not risks.
...
Fifteen entries. Among them: the first probe-control criterion required non-overlap of two independent intervals, which cannot pass for a paired comparison — replaced by the paired test, original FAILs retained; the validation split (117 positives) cannot rank probe checkpoints, so the untrained head sits inside every selection interval — fixed-final-step selection adopted under a registered criterion; the "20 tokens per parameter" heuristic was carried as a planning assumption that never entered the record; a pre-registered attribution clause named one farm where the measurement was made on three; a brief's premise contradicted its registration, and the registration governed.

---

## What is open

- Abstention: graceful degradation under channel loss is **not established** (ADR-0028's H2 is INCONCLUSIVE at this budget and severity). Recalibration on recent data is untested, and until it is tested the probabilities are rankings, not risks.
