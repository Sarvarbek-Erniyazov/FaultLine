...
| Model | Decoder-only causal transformer: RMSNorm, GELU MLP at 4×, tied output head, learned positions, no dropout. A four-rung ladder (S0–S3: 0.31M / 1.18M / 3.54M / 9.96M backbone parameters). **S2 — 8 layers, d_model 192, 8 heads, context 2,048 — is the rung every result below uses**, fixed for comparability across arms rather than selected for performance. | `src/faultline/model/`, `configs/model/ladder_v0.yaml` |
...
- Two withdrawn ablations (`joint_status_raw`, `joint_no_txt`) are revisitable: they would test whether raw status casing and the unpaired narrative corpus contribute anything. ADR-0026 §1 admits them only under a new registration against a named write-up sentence. Its rationale — that an instrument which cannot read the text cannot discriminate between backbones that differ in their text — implies such a registration would read them through the H1′ read-out; that inference is not itself registered.
