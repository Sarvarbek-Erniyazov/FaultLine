...
Every SCADA file goes through the repeated-label rule: drop the rows null in every ingested channel, assert that no (label, column) then holds two values, and collapse to one row per label. `rows_raw` is rows read, `rows_after_null_drop` is rows carrying any ingested value, `labels_distinct` is distinct labels read (label and station, where one file holds every turbine), `rows_out` is rows kept. A file that does not repeat shows `rows_raw == labels_distinct`; a file whose three counts agree also had no row without an ingested value. The assertion held for every file below, or the run would have stopped.
