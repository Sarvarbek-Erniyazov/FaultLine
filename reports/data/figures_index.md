...
| generated (UTC) | 2026-09-24T16:15:43+00:00 |
| git_sha | b6598972787e664f63be9c2b5089c510aace4505 |
| generated by | faultline model figures |

## 1. The set

Ordered by the weight of the sentence each figure carries in the write-up, not by figure number.
...
| F7. Abstention under channel masking (H2) | `fig7_abstention.svg` | Seed-disagreement abstention ranks risk better than chance, but under masking it gives up coverage and selective risk still rises: graceful degradation is not established. |
...
Design (a) is the registered last-position probe; design (d) is ADR-0026's text-aware read-out. The two hollow rows are (d)'s controls: the random-init backbone is the floor the gate had to clear, and the telemetry-only backbone, whose text embedding rows collapsed onto one shared vector, is read with the same head. The standing caveat applies: forward-in-time, same sites; not a site-shift result.
...
### F7. Abstention under channel masking (H2)

![F7. Abstention under channel masking (H2)](fig7_abstention.svg)

**Seed-disagreement abstention ranks risk better than chance, but under masking it gives up coverage and selective risk still rises: graceful degradation is not established.**

ADR-0028, H2. The operating model is the three-seed ensemble of the text-aware read-out; windows are ranked by seed disagreement, and the threshold and the disagreement cut-off were fixed on the 2021 validation split before any test file was read. Masking 8 of the 12 core channels moves the whole risk–coverage curve up. At the fixed operating point coverage falls, -0.0797 [-0.0841, -0.0753], and selective risk rises, +0.0053 [+0.0033, +0.0074], against a registered smallest effect of 0.005: INCONCLUSIVE. Below, the shaded band is the clean selective risk plus that smallest effect. Selective risk steps up at the first severity; coverage does not fall until later. The standing caveat applies: forward-in-time, same sites; not a site-shift result.

Sources:

- `reports/data/abstention_v0_20260924.json`
...
## 3. The streaming traces, and the two rules that chose them
...
| turbine-year | event starts | windows | its own positive rate | its own AUPRC | its interval against that rate |
| --- | --- | --- | --- | --- | --- |
| Kelmarsh 5, 2023 | 162 | 8,614 | 0.1659 | 0.1876 [0.1321, 0.2540] | contains its own positive rate |
| Kelmarsh 4, 2023 | 16 | 8,562 | 0.0429 | 0.0473 [0.0255, 0.0871] | contains its own positive rate |

Each AUPRC above describes its own turbine-year, and is comparable with nothing else in this file: not with the other trace, and not with the pooled gate numbers. A single turbine-year holds a small fraction of the pooled split's 5,312 positive windows, so its interval is far wider, and each trace's own report states its positive count and how much wider. An AUPRC only means anything against the base rate it was measured on, which is why each row carries its own. The standing caveat applies: forward-in-time, same site; not a site-shift result.

Sources:

- `reports/data/stream_traces_v0.json`
- `reports/data/stream_trace_kelmarsh_5_2023.md`
- `reports/data/stream_trace_kelmarsh_4_2023.md`
