...
from this repository plus the public archive, without the archives ever being
committed.
...
card at all. The few fields that are copied from the source specification rather
than measured say so on the card.
"""
...
#: The ``**VERDICT** - why`` line an inventory report writes. Matched generically
#: rather than against a fixed list, so a verdict label added to the inspector is
#: carried onto the card instead of silently degrading to "no verdict line found".
_VERDICT_LINE = re.compile(r"^\*\*((?:UN)?VERIFIED[^*]*?)\*\*\s*-\s*(.*)$", re.M)
...
def build_card(source: str, spec: SourceSpec, paths: ProjectPaths) -> Path:
    ...
    parts.append(
        section(
            ...
            + "\n_Hand-written: copied from the provider's record metadata into "
            "`configs/data/sources_telemetry.yaml`, not generated from the staged files._",
        )
    )
    ...
    parts.append(
        section(
            ...
            f"**Free-text verdict: {verdict}**\n\n{rationale}\n\n"
            "This is the field that decides whether the paired text in this record can carry "
            "a language model, or whether it only supplies labels and structure (ADR-0001). "
            "The verdict is one of `VERIFIED no` (codes only, or a code book of recurring "
